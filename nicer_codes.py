...
def main():
    ...
    stacked_times, stacked_counts, stacked_softs, stacked_hards, index_in = stacker(shortened_list)
    stacked_times, bkg_stacked_counts, bkg_stacked_softs, bkg_stacked_hards, index_in = stacker(shortened_list)

    stacked_counts = stacked_counts - bkg_stacked_counts
    print(index_in)


    soft_in_eclipse = [sum(stacked_softs[:index_in])]
    hard_in_eclipse = [sum(stacked_hards[:index_in])]
    soft_out_eclipse = [sum(stacked_softs[index_in+30:])]
    hard_out_eclipse = [sum(stacked_hards[index_in+30:])]

    bkg_soft_in_eclipse = [sum(bkg_stacked_softs[:index_in+10])]
    bkg_hard_in_eclipse = [sum(bkg_stacked_hards[:index_in+10])]
    bkg_soft_out_eclipse = [sum(bkg_stacked_softs[index_in+30:])]
    bkg_hard_out_eclipse = [sum(bkg_stacked_hards[index_in+30:])]

    print(soft_out_eclipse, hard_out_eclipse, bkg_soft_out_eclipse, bkg_hard_out_eclipse)
    plt.rcParams.update({
            'font.sans-serif': 'Times New Roman',
            'font.family': 'serif'
            })

    global_median_in, global_upper_in, global_lower_in = BEHR_passer(soft_in_eclipse, hard_in_eclipse, bkg_soft_in_eclipse, bkg_hard_in_eclipse, "STACKED")
    global_median_out, global_upper_out, global_lower_out = BEHR_passer(soft_out_eclipse, hard_out_eclipse, bkg_soft_out_eclipse, bkg_hard_out_eclipse, "STACKED")

    print('global')
    print(global_median_in, global_upper_in, global_lower_in)
    print(global_median_out, global_upper_out, global_lower_out)
    ...
    plt.axvline(x=stacked_times[index_in+10])

    plt.axvline(x=stacked_times[index_in+30])
    plt.xlabel("Time [s]")
    ...
    plt.xlabel("Time [ks]",fontsize=40)
    plt.ylabel("Normalized Flux",fontsize=40)
...
def stacker(egress_list):
    ...
    return times,combined_sum, combined_softs, combined_hards, index_in
...
def counts_grabber(binsize,lightcurve):
    ...
    print("INFO",lightcurve)
    print("TIME",abs_times[0])
    print("exp time",(times[-1]-times[0]))
